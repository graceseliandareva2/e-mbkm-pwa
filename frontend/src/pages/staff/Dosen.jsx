...
// PERUBAHAN (item #6): halaman CRUD Data Dosen ini dipindah dari
// pages/kaprodi/Dosen.jsx -- sekarang cuma bisa diakses role staff_akademik
// (backend POST/PUT /staff/dosen). Halaman kaprodi versi lama sudah jadi
// read-only (pages/kaprodi/Dosen.jsx).
//
// PERUBAHAN (fitur Dosen PA): tambah toggle "Jadikan Dosen PA" di modal
// Tambah & Edit, terhubung ke field `is_dosen_pa` (sudah didukung backend
// di tambahDosen/updateDosen). Juga tambah badge kecil "PA" di kolom Nama
// pada tabel supaya kelihatan dosen mana saja yang jadi Dosen PA.
...
export default function StaffDosen() {
  ...
  const handleTambahSubmit = async () => {
    ...
    try {
      const res = await api.post('/staff/dosen', tambahForm)
  ...
  const handleEditSubmit = async () => {
    ...
    setEditLoading(true)
    try {
      const res = await api.put(`/staff/dosen/${editId}`, editForm)
      toast.success(res.data.message || 'Data dosen berhasil diperbarui!')
      setShowEdit(false)
      setEditForm({ ...emptyForm, is_active: true })
      setEditId(null)
      fetchDosen()
    } catch (err) {
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Kelola data dosen pembimbing</p>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between py-3 px-4 bg-gray-50 rounded-xl border border-gray-200">
                <div>
                  <p className="text-sm font-semibold text-gray-700">Jadikan Dosen PA</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    {tambahForm.is_dosen_pa ? 'Dosen ini akan muncul sebagai pilihan Dosen PA' : 'Bukan Dosen Pembimbing Akademik'}
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setTambahForm(prev => ({ ...prev, is_dosen_pa: !prev.is_dosen_pa }))}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none
                    ${tambahForm.is_dosen_pa ? 'bg-indigo-500' : 'bg-gray-300'}`}
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform
                    ${tambahForm.is_dosen_pa ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-bold text-gray-800">Edit Dosen</h2>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between py-3 px-4 bg-gray-50 rounded-xl border border-gray-200">
                <div>
                  <p className="text-sm font-semibold text-gray-700">Jadikan Dosen PA</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    {editForm.is_dosen_pa ? 'Dosen ini muncul sebagai pilihan Dosen PA' : 'Bukan Dosen Pembimbing Akademik'}
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setEditForm(prev => ({ ...prev, is_dosen_pa: !prev.is_dosen_pa }))}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none
                    ${editForm.is_dosen_pa ? 'bg-indigo-500' : 'bg-gray-300'}`}
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform
                    ${editForm.is_dosen_pa ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>
              <div className="flex gap-3 pt-2">
