...
const getMahasiswaBimbingan = async (req, res) => {
  try {
    const [dsn] = await db.query("SELECT id FROM dosen WHERE user_id = ?", [req.user.id]);
    if (!dsn.length) return res.status(404).json({ message: "Data dosen tidak ditemukan." });
...
const berikanPenilaian = async (req, res) => {
  try {
    ...
    const {
      ...
      nilai_kesesuaian, nilai_proyek, nilai_evaluasi, nilai_laporan, nilai_presentasi,
      catatan,
    } = req.body;

    if (!mahasiswa_id || !periode_id) {
      return res.status(400).json({ message: "mahasiswa_id dan periode_id wajib diisi." });
    }

    const nilai_akhir = (
      parseFloat(nilai_kesesuaian) * 0.15 +
      parseFloat(nilai_proyek)     * 0.30 +
      parseFloat(nilai_evaluasi)   * 0.15 +
      parseFloat(nilai_laporan)    * 0.20 +
      parseFloat(nilai_presentasi) * 0.20
    ).toFixed(2);
    ...
    if (existing.length) {
      await db.query(
        ...
        WHERE id=?`,
        [nilai_kesesuaian, nilai_proyek, nilai_evaluasi, nilai_laporan, nilai_presentasi,
          nilai_akhir, grade, catatan, existing[0].id]
      );
    } else {
      await db.query(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [uuidv4(), mahasiswa_id, dsn[0].id, periode_id,
          nilai_kesesuaian, nilai_proyek, nilai_evaluasi, nilai_laporan, nilai_presentasi,
          nilai_akhir, grade, catatan]
      );
    }

    const [mhs] = await db.query("SELECT user_id, nama, email FROM mahasiswa WHERE id = ?", [mahasiswa_id]);
    if (mhs.length) {
      await db.query(
        "INSERT INTO notifikasi (id, user_id, judul, pesan, tipe) VALUES (?, ?, ?, ?, ?)",
        [uuidv4(), mhs[0].user_id, "Nilai Akhir",
          `Dosen pembimbing telah memberikan nilai akhir kamu. Nilai: ${nilai_akhir} (${grade})`, "sukses"]
      );
    }
...
const eksporPenilaianPDF = async (req, res) => {
  try {
    ...
    doc.moveDown(2);
    doc.font('Helvetica').fontSize(10)
      .text(`Dicetak pada: ${new Date().toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' })}`, { align: 'right' });
...
const eksporSemuaPenilaianPDF = async (req, res) => {
  try {
    const [dsn] = await db.query("SELECT id, nama FROM dosen WHERE user_id = ?", [req.user.id]);
    if (!dsn.length) return res.status(404).json({ message: "Data dosen tidak ditemukan." });
    ...
    doc.text('Institut Teknologi & Bisnis Sabda Setia', { align: 'center' });
    doc.moveDown(0.5);
    doc.font('Helvetica').text(`Dosen Pembimbing : ${dsn[0].nama}`, { align: 'center' });
...
const verifikasiDokumen = async (req, res) => {
  try {
    ...
    if (dok[0].jenis === 'laporan_akhir' && dok[0].status !== 'disetujui_kaprodi') {
      return res.status(400).json({
        message: "Laporan Akhir harus disetujui Kaprodi terlebih dahulu sebelum dapat diverifikasi Dosen."
      });
    }
    ...
    if (mhs.length) {
      ...
      await db.query(
        "INSERT INTO notifikasi (id, user_id, judul, pesan, tipe) VALUES (?, ?, ?, ?, ?)",
        [uuidv4(), mhs[0].user_id, "Status Dokumen", pesan,
...
const berikanFeedback = async (req, res) => {
  try {
    const [dsn] = await db.query("SELECT id FROM dosen WHERE user_id = ?", [req.user.id]);
    const { mahasiswa_id, periode_id, referensi_id, referensi_tipe, isi_feedback } = req.body;
    ...
    await db.query(
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [uuidv4(), mahasiswa_id, dsn[0].id, periode_id,
        referensi_id || null, referensi_tipe || "logbook", isi_feedback]
    );
    const [mhs] = await db.query("SELECT user_id FROM mahasiswa WHERE id = ?", [mahasiswa_id]);
    if (mhs.length) {
      await db.query(
        "INSERT INTO notifikasi (id, user_id, judul, pesan, tipe) VALUES (?, ?, ?, ?, ?)",
        [uuidv4(), mhs[0].user_id, "Feedback Baru",
          `Dosen pembimbing memberikan feedback: ${isi_feedback}`, "info"]
      );
    }
